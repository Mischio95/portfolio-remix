...
import { getFornitori } from "~/models/fornitori.server";
import { getExpenses } from "~/routes/api.expenses";
import { getCompetitori } from "~/models/competitor.server";
...
export const action: ActionFunction = async ({ request }) => {
  try {
    ...
    const competitori = await getCompetitori();
    console.log("Competitori recuperati:", competitori);
    ...
    const htmlContent = `
      ...
      <html lang="it">
        <head>
          <meta charset="UTF-8">
          <meta name="viewport" content="width=device-width, initial-scale=1.0">
          <title>Report Fornitori, Competitori e Spese</title>
          <style>
            body {
              font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;
              line-height: 1.6;
        ...
        <body>
          ...
          <h1>Report Competitori</h1>
          <table>
            <thead>
              <tr>
                <th>Nome Azienda</th>
                <th>Sito Web</th>
                <th>Descrizione Prodotti Venduti</th>
              </tr>
            </thead>
            <tbody>
              ${competitori
                .map(
                  (competitore) => `
                <tr>
                  <td>${competitore.nomeAzienda}</td>
                  <td>${competitore.sitoWeb}</td>
                  <td>${competitore.descrizioneProdottiVenduti || "N/A"}</td>
                </tr>
              `
                )
                .join("")}
